...
class VGG19PerceptualLoss(torch.nn.Module):
    def __init__(self, resize=False):
        ...
        blocks = []
        VGG19 = torchvision.models.vgg19(pretrained=True).to(cfg.device)
        ...
        self.resize = resize
        self.register_buffer("mean", torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1).to(cfg.device))
        self.register_buffer("std", torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1).to(cfg.device))
